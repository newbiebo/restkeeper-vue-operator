<template>
  <div class="app-container">
    ...
    <el-table
      ...
      <el-table-column :label="$t('table.username')" prop="username" min-width="150px" />
      <el-table-column :label="$t('table.realName')" prop="realName" width="90px" />
      <el-table-column :label="$t('table.sex')" prop="sex" width="110px" :formatter="formatSex"/>
      <el-table-column :label="$t('table.mobil')" prop="mobil" width="110px" />
      <el-table-column :label="$t('table.email')" prop="email" width="180px" />
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible" width="60%" top="10px">
      <el-form  size="small" ref="dataForm" :rules="rules" :model="temp" label-position="right"  label-width="80px" style="width: 400px; margin-left:30px;">
        ...
        <el-form-item :label="$t('table.hasRoleIds')" prop="hasRoleIds">
          <el-select  v-model="temp.hasRoleIds" clearable class="filter-item" multiple :placeholder="$t('table.PleaseSelect')">
            <el-option v-for="item in calendarRoleIdOptions" :key="item.id" :label="item.roleName" :value="item.id">
            </el-option>
          </el-select>
        </el-form-item>

        <el-form-item :label="$t('table.enterpriseId')" prop="enterpriseId">
          <el-select  v-model="temp.enterpriseId" clearable class="filter-item" :placeholder="$t('table.PleaseSelect')">
            <el-option
              v-for="item in calendarEnterpriseIdOptions"
...
<script>
import { fetchList,initRoleIdOptions,createInfo,updateInfo,deleteInfo,changeEnableFlag } from '@/api/user'
...
export default {
  ...
  methods: {
    ...
    //初始化角色下拉框
    initRoleIdOptions(){
      initRoleIdOptions().then(response => {
        this.calendarRoleIdOptions = response.datas
      })
    },
    initEnableFlag() {
